...
│   │
│   ├── widgets/
│   │   ├── categoryFilter.js  # 分类筛选组件（v2 动态分类，从数据库提取）
...
│   ├── words_ky.json          # 考研词库
│   ├── words_toefl.json       # 托福词库
│   ├── words_ielts.json       # 雅思词库
│   ├── words_gre.json         # GRE 词库
│   ├── words_gk.json          # 高考词库
│   └── words_zk.json          # 中考词库
│
├── scripts/
│   ├── generate_manual.js     # 生成使用手册 DOCX 脚本
...
└── docs/
    ...
    ├── future_improvements.md # 后续改进方向
    ├── WordWiz_使用手册.docx   # 使用手册
    └── ai/                    # ← AI 辅助上下文
        ├── context.md
        ├── architecture.md
        ├── rules.md
          ...
          └→ 按顺序加载 JS 文件
              └→ DOMContentLoaded 触发
                  └→ app.js: new WordWizApp().init()
                      ...
                      6. NotificationHelper.startReminderChecker() → 启动提醒
                      7. hash = '#/home'            → 触发首页渲染
```
...
window.AppState = {
    home: {
        ...
        shuffledWords: null    // shuffled 模式下存储的单词 ID 数组
    },
    favorites: {
        category: '全部',
        sortMode: 'default',
        shuffledWords: null    // 混序状态下存储的 ID 顺序
    }
};
```
...
| 变量 | 类型 | 说明 |
|------|------|------|
| `window.WordDB` | `WordDatabase` 实例 | 数据库操作入口（挂载 40+ DAO 方法） |
| `window.WordModel` | 类 | 数据模型工厂 + 多归属工具方法 |
| `window.WordCard` | 类 | 单词行组件（静态） |
| `window.UnitCard` | 类 | 单元卡片组件（静态） |
| `window.CategoryFilter` | 类 | 分类筛选组件（v2 动态分类） |
...
| `window.WordSorter` | 类 | 通用排序模块（6 种模式） |
...
| 动态分类筛选 | `categoryFilter.js` | 从数据库提取 |
| 6 种排序模式 | `sorter.js` | 默认/熟悉度↑/熟悉度↓/A-Z/Z-A/随机混序 |
...
| 功能 | 文件 | 说明 |
|------|------|------|
| 3 种答题模式 | `challenge.js` | 四选一 / 汉→英（首字母提示） / 英→汉 |
| 难度筛选 | `challenge.js` | 简单（熟悉度≥3）/ 普通 / 困难（熟悉度≤2） |
...
| 挑战历史 | `result.js` | 最近 20 条历史，设置页可查看 |
| 成就触发 | `achievements.js` | 初次挑战/满分王/速度之星/连对达人等 |
...
| 每日提醒 | `notifications.js` | 桌面通知 + Toast + 提示音，可设时间 |
| 成就墙 | `achievements.js` | 10 个成就展示 |
...
```
 ...
 2. models/word.js             (数据模型 + 多归属工具)
 3. connection.js              (数据库基类 + 建表 + 初始化 + 预置数据)
...
⚠️ **顺序不能乱** — 依赖关系严格。加载改动后需要同步递增 `?v=` 参数防缓存。

---

## 九、多词书拆分（scripts/split_csv.js）

基于 ECDICT (MIT License) 的 tag 列，生成 8 本词书：

| 文件 | 内容 | 单词数 | 单元数 |
|------|------|--------|--------|
| `words_cet4.json` | 四级词汇 (cet4) | ~4,600 | 46 |
| `words_cet6.json` | 六级词汇 (cet6) | ~4,350 | 44 |
...
> 运行 `node scripts/split_csv.js` 生成
...
5ccc39c 综合复盘优化：完善挑战模式/错题集/解析器 + 移动端适配 + 文档更新
